package cn.shudong.uaa.service;

import cn.shudong.core.exception.GlobalException;
import cn.shudong.core.exception.GlobalExceptionCode;
import cn.shudong.core.util.JWTUtil;
import cn.shudong.uaa.dao.AuthorityDao;
import cn.shudong.uaa.dao.RoleDao;
import cn.shudong.uaa.dao.UserDao;
...
public class LoginService {
    ...
    public LoginVo login(String username, String password) {
        ...
        //查询用户所拥有的角色和权限的标识集合
        List<String> roleArr = this.roleDao.selectByUserId(user.getId(), Boolean.FALSE);
        List<String> authorityArr = this.authorityDao.selectByUserId(user.getId(), Boolean.FALSE);

        //生成 token
        String jwt = JWTUtil.generate(String.valueOf(user.getId()), user.getUsername(), roleArr, authorityArr, username, this.jwtSign, this.jwtExpiration);
